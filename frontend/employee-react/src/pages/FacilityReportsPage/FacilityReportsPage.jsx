...
import "./FacilityReportsPage.css";
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/reports/${houseId}`
      );
      setReports(response.data); // Assume response.data is an array of reports
    } catch (err) {
      console.error(err);
      setError(
        err.response?.data?.message || "Failed to fetch facility reports."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Container style={{ marginTop: "5vh" }} className="facility-containter">
      ...
      <Modal
        ...
        onClose={() => setViewCommentsModalOpen(false)}
      >
        <Paper style={{ margin: "20vh auto", padding: "20px", width: "400px" }}>
          ...
          {comments.length > 0 ? (
            comments.map((comment, index) => (
              ...
              >
                ...
                <strong>At:</strong>{" "}
                {new Date(comment.updated_at || Date.now()).toLocaleString()}{" "}
                <br />
